...
class ExternalCitationsTest(TestCase):
    @patch('regulations.generator.layers.external_citation.'
           'ExternalCitationLayer.generate_fdsys_href_tag')
    ...
    def test_public_law_link(self, generate_fdsys_href_tag):
        ...
        parameters = generate_fdsys_href_tag.call_args[0][1]
        self.assertEqual(
            parameters.keys(), ['collection', 'lawnum', 'congress', 'lawtype'])
    ...
    def test_cfr_link(self, generate_fdsys_href_tag):
        ...
        self.assertEqual(
            parameters.keys(),
            ['titlenum', 'link-type', 'collection', 'partnum'])
    ...
    def test_citation_type_to_generator(self):
        ...
        self.assertEqual(generator,
                         ExternalCitationLayer.generate_statutes_at_large_link)

        citation_type = 'PUBLIC_LAW'
        generator = layer.citation_type_to_generator(citation_type)
        self.assertEqual(generator,
                         ExternalCitationLayer.generate_public_law_link)
